        ...
        <template v-if="showHeader">
            ...
            <section class="page-section mb-3" >
                <div class="flex flex-wrap justify-between items-center gap-3">
                    <div  class="col-span-full " >
                        <Button label="Add Product" icon="pi pi-plus"  @click="app.openPageDialog({ component: ProductsAddPage, url: `/products/add` , closeBtn: true })"  class="p-button w-full bg-primary "  />
                    </div>
                    <div  class="col-span-full " >
                    ...
                    <div  class="col-span-full comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/products'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"  :value="records" dataKey="product_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="product_id" header="Product Id" >
                                            <template #body="{ data, index }">
                                                <router-link class="p-button-text" :to="`/products/view/${data.product_id}`">
                                                    {{ data.product_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="stock_quantity" header="Stock Quantity" >
                                            <template #body="{ data, index }">
                                                {{ data.stock_quantity }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-between items-center">
                                        <div class="flex gap-2 flex-grow-0 mt-3">
                                            ...
                                            <import-data label="Select a file to import" ref="dataimport" upload-path="products/importdata" @importComplete="importComplete">
...
<script setup>
	...
	import ProductsAddPage from 'src/pages/products/add.vue';
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/products/view/${data.product_id}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/products/edit/${data.product_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.product_id) },
			icon: "pi pi-trash"
		}
	]
	}
