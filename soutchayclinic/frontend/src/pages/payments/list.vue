        ...
        <template v-if="showHeader">
            ...
            <section class="page-section mb-3" >
                <div class="flex flex-wrap justify-between items-center gap-3">
                    <div  class="col-span-full " >
                        <Button label="Add Payment" icon="pi pi-plus"  @click="app.openPageDialog({ component: PaymentsAddPage, url: `/payments/add` , closeBtn: true })"  class="p-button w-full bg-primary "  />
                    </div>
                    <div  class="col-span-full " >
                    ...
                    <div  class="col-span-full comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/payments'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"  :value="records" dataKey="payment_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="payment_id" header="Payment Id" >
                                            <template #body="{ data, index }">
                                                <router-link class="p-button-text" :to="`/payments/view/${data.payment_id}`">
                                                    {{ data.payment_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-between items-center">
                                        <div class="flex gap-2 flex-grow-0 mt-3">
                                            ...
                                            <import-data label="Select a file to import" ref="dataimport" upload-path="payments/importdata" @importComplete="importComplete">
...
import PaymentsAddPage from 'src/pages/payments/add.vue';
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/payments/view/${data.payment_id}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/payments/edit/${data.payment_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
