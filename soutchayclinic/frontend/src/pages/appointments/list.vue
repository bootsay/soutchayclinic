        ...
        <template v-if="showHeader">
            ...
            <section class="page-section mb-3" >
                <div class="flex flex-wrap justify-between items-center gap-3">
                    <div  class="col-span-full " >
                        <Button label="Add Appointment" icon="pi pi-plus"  @click="app.openPageDialog({ component: AppointmentsAddPage, url: `/appointments/add` , closeBtn: true })"  class="p-button w-full bg-primary "  />
                    </div>
                    <div  class="col-span-full " >
                    ...
                    <div  class="col-span-full comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/appointments'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"  :value="records" dataKey="appointment_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="appointment_id" header="Appointment Id" >
                                            <template #body="{ data, index }">
                                                <router-link class="p-button-text" :to="`/appointments/view/${data.appointment_id}`">
                                                    {{ data.appointment_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="patient_id" header="Patient Id" >
                                            <template #body="{ data, index }">
                                                <Button variant="text" icon="pi pi-eye" label="Patients" v-if="data.patient_id" @click="app.openPageDialog({ component: PatientsViewPage, url: `/patients/view/${data.patient_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="service_id" header="Service Id" >
                                            <template #body="{ data, index }">
                                                <Button variant="text" icon="pi pi-eye" label="Services" v-if="data.service_id" @click="app.openPageDialog({ component: ServicesViewPage, url: `/services/view/${data.service_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        <Column  field="staff_id" header="Staff Id" >
                                            <template #body="{ data, index }">
                                                <Button variant="text" icon="pi pi-eye" label="Staff" v-if="data.staff_id" @click="app.openPageDialog({ component: StaffViewPage, url: `/staff/view/${data.staff_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-between items-center">
                                        <div class="flex gap-2 flex-grow-0 mt-3">
                                            ...
                                            <import-data label="Select a file to import" ref="dataimport" upload-path="appointments/importdata" @importComplete="importComplete">
...
<script setup>
	...
	import AppointmentsAddPage from 'src/pages/appointments/add.vue';
import PatientsViewPage from 'src/pages/patients/view.vue';
import ServicesViewPage from 'src/pages/services/view.vue';
import StaffViewPage from 'src/pages/staff/view.vue';
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/appointments/view/${data.appointment_id}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/appointments/edit/${data.appointment_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.appointment_id) },
			icon: "pi pi-trash"
		}
	]
	}
