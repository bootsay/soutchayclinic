                                        ...
                                        <div class="grid grid-cols-12 gap-4">
                                            ...
                                            <div class="col-span-full">
                                                <div class="grid grid-cols-4 md:gap-3">
                                                    ...
                                                    <div class="col-span-full md:col-span-3">
                                                        <Textarea :class="getErrorClass('treatment_result')" class="w-full" rows="5"  v-model="formData.treatment_result" placeholder="Enter Treatment Result"    type="textarea">
                                                        </Textarea>
                                                        <small v-if="isFieldValid('treatment_result')" class="p-error">{{ getFieldError('treatment_result') }}</small> 
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<script setup>
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'treatmentrecordsedit',
		},
		pagePath: {
			type : String,
			default : 'treatmentrecords/edit',
		},
		apiPath: {
			type: String,
			default: 'treatmentrecords/edit',
	...
	const formDefaultValues = Object.assign({
		appointment_id: "", notes: "", treatment_result: "", 
	}, props.pageData);
	...
	function afterSubmit(response) {
		...
		if(app.isDialogOpen()){
			...
			app.navigateTo(`/treatmentrecords`);
		}
	...
	onMounted(()=>{
		const pageTitle = "Edit Treatment Record";
