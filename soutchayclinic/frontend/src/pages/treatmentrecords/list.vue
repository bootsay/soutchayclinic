        ...
        <template v-if="showHeader">
            ...
            <section class="page-section mb-3" >
                <div class="flex flex-wrap justify-between items-center gap-3">
                    <div  class="col-span-full " >
                        <Button label="Add Treatment Record" icon="pi pi-plus"  @click="app.openPageDialog({ component: TreatmentrecordsAddPage, url: `/treatmentrecords/add` , closeBtn: true })"  class="p-button w-full bg-primary "  />
                    </div>
                    <div  class="col-span-full " >
                    ...
                    <div  class="col-span-full comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/treatmentrecords'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"  :value="records" dataKey="record_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="record_id" header="Record Id" >
                                            <template #body="{ data, index }">
                                                <router-link class="p-button-text" :to="`/treatmentrecords/view/${data.record_id}`">
                                                    {{ data.record_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        <Column  field="appointment_id" header="Appointment Id" >
                                            <template #body="{ data, index }">
                                                <Button variant="text" icon="pi pi-eye" label="Appointments" v-if="data.appointment_id" @click="app.openPageDialog({ component: AppointmentsViewPage, url: `/appointments/view/${data.appointment_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="treatment_result" header="Treatment Result" >
                                            <template #body="{ data, index }">
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-between items-center">
                                        <div class="flex gap-2 flex-grow-0 mt-3">
                                            ...
                                            <import-data label="Select a file to import" ref="dataimport" upload-path="treatmentrecords/importdata" @importComplete="importComplete">
...
import TreatmentrecordsAddPage from 'src/pages/treatmentrecords/add.vue';
	...
	const props = defineProps({
		...
		routeName: {
			type: String,
			default: 'treatmentrecordslist',
		},
		apiPath: {
			type: String,
			default: 'treatmentrecords/index',
		},
		autoLoad: {
			type: Boolean,
			default: true,
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/treatmentrecords/view/${data.record_id}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/treatmentrecords/edit/${data.record_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
	...
	onMounted(()=>{ 
		const pageTitle = "Treatment Records";
