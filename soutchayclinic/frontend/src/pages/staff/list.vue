        ...
        <template v-if="showHeader">
            ...
            <section class="page-section mb-3" >
                <div class="flex flex-wrap justify-between items-center gap-3">
                    <div  class="col-span-full " >
                        <Button label="Add Staff" icon="pi pi-plus"  @click="app.openPageDialog({ component: StaffAddPage, url: `/staff/add` , closeBtn: true })"  class="p-button w-full bg-primary "  />
                    </div>
                    <div  class="col-span-full " >
                    ...
                    <div  class="col-span-full comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/staff'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"  :value="records" dataKey="staff_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="staff_id" header="Staff Id" >
                                            <template #body="{ data, index }">
                                                <router-link class="p-button-text" :to="`/staff/view/${data.staff_id}`">
                                                    {{ data.staff_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="phone" header="Phone" >
                                            <template #body="{ data, index }">
                                                <a class="p-button-text" :href="`tel:${data.phone}`">
                                                    {{ data.phone }}
                                                </a>
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-between items-center">
                                        <div class="flex gap-2 flex-grow-0 mt-3">
                                            ...
                                            <import-data label="Select a file to import" ref="dataimport" upload-path="staff/importdata" @importComplete="importComplete">
...
<script setup>
	...
	import StaffAddPage from 'src/pages/staff/add.vue';
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/staff/view/${data.staff_id}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/staff/edit/${data.staff_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.staff_id) },
			icon: "pi pi-trash"
		}
	]
	}
