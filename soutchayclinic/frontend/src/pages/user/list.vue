        ...
        <template v-if="showHeader">
            ...
            <section class="page-section mb-3" >
                <div class="flex flex-wrap justify-between items-center gap-3">
                    <div  class="col-span-full " >
                        <Button label="Add User" icon="pi pi-plus"  @click="app.openPageDialog({ component: UserAddPage, url: `/user/add` , closeBtn: true })"  class="p-button w-full bg-primary "  />
                    </div>
                    <div  class="col-span-full " >
                    ...
                    <div  class="col-span-full comp-grid" >
                        ...
                        <div >
                            ...
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"  :value="records" dataKey="userid" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="userid" header="Userid" >
                                            <template #body="{ data, index }">
                                                <router-link class="p-button-text" :to="`/user/view/${data.userid}`">
                                                    {{ data.userid }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="phonenumber" header="Phonenumber" >
                                            <template #body="{ data, index }">
                                                <a class="p-button-text" :href="`tel:${data.phonenumber}`">
                                                    {{ data.phonenumber }}
                                                </a>
                                            </template>
                                        </Column>
                                        <Column  field="user_role_id" header="User Role Id" >
                                            <template #body="{ data, index }">
                                                <Button variant="text" icon="pi pi-eye" label="Roles" v-if="data.user_role_id" @click="app.openPageDialog({ component: RolesViewPage, url: `/roles/view/${data.user_role_id}` , closeBtn: true })" />
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-between items-center">
                                        <div class="flex gap-2 flex-grow-0 mt-3">
                                            ...
                                            <import-data label="Select a file to import" ref="dataimport" upload-path="user/importdata" @importComplete="importComplete">
...
<script setup>
	...
	import RolesViewPage from 'src/pages/roles/view.vue';
import UserAddPage from 'src/pages/user/add.vue';
