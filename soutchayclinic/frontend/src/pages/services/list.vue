        ...
        <template v-if="showHeader">
            ...
            <section class="page-section mb-3" >
                <div class="flex flex-wrap justify-between items-center gap-3">
                    <div  class="col-span-full " >
                        <Button label="Add Service" icon="pi pi-plus"  @click="app.openPageDialog({ component: ServicesAddPage, url: `/services/add` , closeBtn: true })"  class="p-button w-full bg-primary "  />
                    </div>
                    <div  class="col-span-full " >
                    ...
                    <div  class="col-span-full comp-grid" >
                        ...
                        <div >
                            <template v-if="showBreadcrumbs && $route.query.tag && !isSubPage">
                                <Breadcrumb :home="{icon: 'pi pi-home', to: '/services'}" :model="pageBreadCrumb" />
                            </template>
                            <!-- page records template -->
                            <div class="page-records"  >
                                <DataTable :lazy="true"   :loading="loading"    v-model:selection="selectedItems"  :value="records" dataKey="service_id" @sort="onSort($event)" class=" p-datatable-sm" :stripedRows ="true" :showGridlines="false" :rowHover="true" responsiveLayout="stack">
                                    <Column selectionMode="multiple" headerStyle="width: 2rem" />
                                        <Column  field="service_id" header="Service Id" >
                                            <template #body="{ data, index }">
                                                <router-link class="p-button-text" :to="`/services/view/${data.service_id}`">
                                                    {{ data.service_id }}
                                                </router-link>
                                            </template>
                                        </Column>
                                        ...
                                        <Column  field="duration_min" header="Duration Min" >
                                            <template #body="{ data, index }">
                                                {{ data.duration_min }}
                                            </template>
                                        </Column>
                                ...
                                <template v-if="showFooter && pageReady">
                                    <div class="flex justify-between items-center">
                                        <div class="flex gap-2 flex-grow-0 mt-3">
                                            ...
                                            <import-data label="Select a file to import" ref="dataimport" upload-path="services/importdata" @importComplete="importComplete">
...
<script setup>
	...
	import ServicesAddPage from 'src/pages/services/add.vue';
	...
	function getActionMenuModel(data){
		...
		{
			label: "View",
			to: `/services/view/${data.service_id}`,
			icon: "pi pi-eye"
		},
		{
			label: "Edit",
			to: `/services/edit/${data.service_id}`,
			icon: "pi pi-pencil"
		},
		{
			label: "Delete",
			command: (event) => { deleteItem(data.service_id) },
			icon: "pi pi-trash"
		}
	]
	}
